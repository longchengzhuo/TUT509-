...
def hist2d_demo(image):  # 二维直方图的显示
    hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)
    hist = cv.calcHist([hsv], [0, 1], None, [100, 100],[0, 180, 0, 256])  # 计算直方图 [0,1] 表示HSV的前俩个通道  [180,256]表示每个通道的取值范围
    print(hist.shape)
...
def back_projection_demo():
    ...
    roiHist = cv.calcHist([roi_hsv], [0, 1], None, [35, 35], [0, 180, 0, 256])
