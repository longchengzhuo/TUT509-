...
# 游戏结束的模板图像
temp_end = cv2.imread('D:/biancheng\opencv python\waigua\end.jpg', 0)
...
# 读取小跳棋模板图像
temple = cv2.imread('D:/biancheng\opencv python\waigua/temple.jpg', 0)
# 获取小跳棋模板图像的高和宽
...
def get_start(img):
    ...
    # 得到小跳棋的中心位置参数
    return max_loc[0] + 39, max_loc[1] + 209
...
def get_end(img):
    ...
    # 第一个顶点的高度
    y_top = np.nonzero([max(row) for row in canny_img[574:]])[0][0] + 574
    # 第一个顶点的宽度
